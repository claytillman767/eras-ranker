...
//   'green' — green circle + white waves; on WHITE backgrounds only
//   'black' — black circle + white waves; on light/colored backgrounds
//   'white' — white circle with the waves cut out as transparent negative space
//             so the underlying background (e.g. Spotify green button) shows
//             through the cutouts. Matches Spotify's official monochrome
//             white-on-color rendering.
//
    ...
    return (
      <svg width={size} height={size} viewBox="0 0 24 24" aria-label="Spotify" style={{ flexShrink: 0 }}>
        <mask id={maskId}>
          ...
          <path d={WAVES_D} fill="black" />
        </mask>
        <circle cx="12" cy="12" r="12" fill="#ffffff" mask={`url(#${maskId})`} />
      </svg>
    );
  }

  const circleFill = variant === 'green' ? '#1DB954' : '#191414';
  return (
    <svg width={size} height={size} viewBox="0 0 24 24" aria-label="Spotify" style={{ flexShrink: 0 }}>
      <circle cx="12" cy="12" r="12" fill={circleFill} />
      <path d={WAVES_D} fill="white" />
    </svg>
  );
}
